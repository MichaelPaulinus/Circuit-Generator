...
while count != numofCircuits:
    ...
    if c1 == 4: # check if component 1 is a bjt so arrangement of following components align with bjt
    ...
    if random.random()>0.5: # for multiple branches
        if c1==1 or c1==2:
            if random.random()>0.5:
                comp5 = elm.Resistor(d='right', at=comp1.end)  # add resistor
            else:
                comp5 = elm.Capacitor(d='right', at=comp1.end) # add capacitor
            d.add(comp5)
    ...
    i = 0
    while os.path.exists("schematic%s.svg" % i):
        i += 1
    d.save("schematic%s.svg" % i, False, 300)
    count += 1